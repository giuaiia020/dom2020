		...
		while( $i < 100) {
			...
			if (!empty($kiwot)) {
				...
				$url = str_replace (' ','-',$kiwot);
                                $domen = $_SERVER['HTTP_HOST'];
				$urlnya = 'http://'.$url.'.'.$domen.'';
$judul = ucwords($kiwot);
				echo '<li><h3><a href="'.$urlnya.'" title="'.$judul.'">'.$judul.'</a></h3></li>';
				echo '<li><h3><a href="'.$urlnya.'/sitemap.xml" title="'.$judul.' Book Index">'.$judul.' Book Index</a></h3></li>';
				echo '<li><h3><a href="'.$urlnya.'/manual.xml" title="'.$judul.' Manual Index">'.$judul.' Manual Index</a></h3></li>';
			}
			$i++;
		}
		echo '';
...
<title><?php echo $domen;?> Website</title>
